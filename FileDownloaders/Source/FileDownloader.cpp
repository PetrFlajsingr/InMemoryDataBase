...
void FileDownloader::addObserver(gsl::not_null<FileDownloadObserver *> observer) { observers.emplace_back(observer); }
...
void FileDownloader::notifyDownloadFailed(std::string_view fileName, std::string_view errorMessage) {
  for (auto observer : observers) {
    observer->onDownloadFailed(fileName, errorMessage);
  }
}
void FileDownloader::notifyDownloadFinished(std::string_view fileName, std::string_view filePath) {
  for (auto observer : observers) {
