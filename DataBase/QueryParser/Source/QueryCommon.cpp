...
DataBase::CondOperator DataBase::tokenToCondOperator(DataBase::Token token) {
  switch (token) {
  case Token::greater:
    return CondOperator::greater;
  case Token::greaterEqual:
    return CondOperator::greaterEqual;
  case Token::less:
    return CondOperator::less;
  case Token::lessEqual:
    return CondOperator::lessEqual;
  case Token::equal:
    return CondOperator::equal;
  case Token::notEqual:
    return CondOperator::notEqual;
  default:
    throw IllegalStateException("Internal error: DataBase::tokenToCondOperator");
  }
}
...
DataBase::ConstType DataBase::tokenToConstType(DataBase::Token token) {
  ...
  case Token::numberFloat:
    return ConstType::floatingPoint;
  case Token::string:
...
DataBase::LogicOperator DataBase::tokenToLogic(DataBase::Token token) {
  switch (token) {
  case Token::logicOr:
    return LogicOperator::logicOr;
  case Token::logicAnd:
    return LogicOperator::logicAnd;
  default:
    throw IllegalStateException("Internal error: DataBase::tokenToConstType");
  }
}
...
DataBase::AgrOperator DataBase::tokenToAgrOperation(DataBase::Token token) {
  switch (token) {
  case Token::sum:
    return AgrOperator::sum;
  case Token::avg:
    return AgrOperator::avg;
  case Token::min:
    return AgrOperator::min;
  case Token::max:
    return AgrOperator::max;
  case Token::count:
    return AgrOperator::count;
  default:
    throw IllegalStateException("Internal error: DataBase::tokenToConstType");
  }
}
