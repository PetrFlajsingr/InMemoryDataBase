...
SCENARIO("Reading csv file using via BaseDataProvider StreamReader interface", "[CsvStreamReader]") {
  GIVEN("Simple csv file") {
    ...
    WHEN("restarting the reading") {
      ...
      THEN("the read contents are the same") {
        ...
        while (csvStreamReader.next()) {
          ...
          std::copy(csvStreamReader.getRow().begin(), csvStreamReader.getRow().end(), std::back_inserter(row));
    ...
    WHEN("Reading column that doesnt match with header") {
      THEN("Throw exception") { REQUIRE_THROWS(csvStreamReaderBad.next()); }
    }
  }
...
SCENARIO("Reading csv file via iterator StreamReader", "[CsvStreamReader]") {
  ...
  GIVEN("A bit complex csv") {
    ...
    WHEN("restarting the reading") {
      ...
      THEN("the read contents are the same") {
        std::vector<std::vector<std::string>> toCheck;
        std::copy(csvStreamReader.begin(), csvStreamReader.end(), std::back_inserter(toCheck));
...
SCENARIO("Writing parsed csv directly to DataWriter using BaseDataProvider StreamReader interface",
...
SCENARIO("Writing parsed csv directly to DataWriter using StreamReader iterator", "[CsvStreamReader][integration]") {
