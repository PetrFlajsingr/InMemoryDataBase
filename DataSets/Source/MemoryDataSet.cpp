  ...
  data.back()->reserve(columnCount);
    ...
    switch (fields[i]->getFieldType()) {
      ...
      case ValueType::Double:
        ...
        break;
      case ValueType::String:data.back()->emplace_back(DataContainer{._string = strdup(record[i].c_str())});
        break;
      case ValueType::Currency:data.back()->emplace_back(DataContainer{._currency = new Currency(record[i])});
        break;
      case ValueType::DateTime:
        data.back()->emplace_back(DataContainer{._dateTime = new DateTime(record[i],
                                                                          DateTimeType::Date)});
        break;
      default:
...
void DataSets::MemoryDataSet::setFieldValues() {
  for (size_t i = 0; i < fields.size(); i++) {
    ...
    switch (fields[i]->getFieldType()) {
      case ValueType::Integer:setFieldData(fields[i].get(), &currentCell._integer);
        break;
      case ValueType::Double:setFieldData(fields[i].get(), &currentCell._double);
        break;
      case ValueType::String:setFieldData(fields[i].get(), currentCell._string);
        break;
      case ValueType::Currency:setFieldData(fields[i].get(), currentCell._currency);
        break;
      case ValueType::DateTime:setFieldData(fields[i].get(), currentCell._dateTime);
        break;
...
void DataSets::MemoryDataSet::sort(SortOptions &options) {
  ...
  bool isInRange = std::all_of(options.options.begin(), options.options.end(), [fieldCount](SortItem &item) {
...
DataSets::BaseField *DataSets::MemoryDataSet::fieldByName(std::string_view name) const {
  ...
  std::string
      errMsg = "Field named \"" + std::string(name) + "\" not found. DataSets::MemoryDataSet::fieldByName";
...
DataContainer &DataSets::MemoryDataSet::getCell(gsl::index row, gsl::index column) {
...
std::shared_ptr<DataSets::MemoryViewDataSet> DataSets::MemoryDataSet::fullView() {
  ...
  auto
      result = std::make_shared<DataSets::MemoryViewDataSet>(getName(), fieldNames, fieldTypes, fieldIndices);
