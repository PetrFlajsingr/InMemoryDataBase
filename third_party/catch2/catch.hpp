...
inline auto operator "" _sr(char const *rawChars,
...
class TestInvokerAsMethod : public ITestInvoker {
 ...
 public:
  TestInvokerAsMethod(void (C::*testAsMethod)()) noexcept : m_testAsMethod(
      testAsMethod) {}
...
std::string stringify(const T &e) {
  return ::Catch::StringMaker<typename std::remove_cv<typename std::remove_reference<
      T>::type>::type>::convert(e);
}
...
class Approx {
 ...
 public:
  ...
  template<typename T, typename = typename std::enable_if<std::is_constructible<
      double,
      T>::value>::type>
...
auto makeMatchExpr(ArgT const &arg,
                   MatcherT const &matcher,
                   StringRef const &matcherString) -> MatchExpr<ArgT,
                                                                MatcherT> {
...
auto random(T const &first, T const &last) -> Generator<T> {
  ...
  return Generator<T>(size,
                      pf::make_unique<GeneratorRandomiser<T>>(std::move(gen),
                                                              size));
}
template<typename T>
...
template<typename... Ts>
auto table(std::initializer_list<std::tuple<Ts...>> &&tuples) -> Generator<std::tuple<
    Ts...>> {
  return values<std::tuple<Ts...>>(std::forward<std::initializer_list<std::tuple<
      Ts...>>>(tuples));
}
...
auto makeGenerators(Generator<T> &&generator,
...
auto generate(SourceLineInfo const &lineInfo,
  ...
  if (!tracker.hasGenerator())
    tracker.setGenerator(pf::make_unique<Generators<UnderlyingType>>(
        generatorExpression()));
...
class IndexTracker : public TrackerBase {
  ...
  int m_index = -1;
 public:
  IndexTracker(NameAndLocation const &nameAndLocation,
               TrackerContext &ctx,
               ITracker *parent,
               int size);
...
bool Approx::equalityComparisonImpl(const double other) const {
  ...
  return marginComparison(m_value, other, m_margin) || marginComparison(m_value,
                                                                        other,
                                                                        m_epsilon
                                                                            * (m_scale
                                                                                + std::fabs(
...
void AssertionHandler::handleMessage(ResultWas::OfType resultType,
                                     StringRef const &message) {
  m_resultCapture.handleMessage(m_assertionInfo,
                                resultType,
                                message,
                                m_reaction);
}
...
class TokenStream {
 ...
 public:
  explicit TokenStream(Args const &args) : TokenStream(args.m_args.begin(),
                                                       args.m_args.end()) {}
...
class ExeName : public ComposableParserImpl<ExeName> {
 ...
 public:
  ...
  auto parse(std::string const &,
    ...
    return InternalParseResult::ok(ParseState(ParseResultType::NoMatch,
                                              tokens));
  }
 ...
 public:
  ...
  auto parse(std::string const &,
    ...
    if (!result)
      ...
      return InternalParseResult::ok(ParseState(ParseResultType::Matched,
                                                ++remainingTokens));
  }
};
...
class Opt : public ParserRefImpl<Opt> {
 ...
 public:
  template<typename LambdaT>
  explicit Opt(LambdaT const &ref) : ParserRefImpl(std::make_shared<
    ...
    if (remainingTokens && remainingTokens->type == TokenType::Option) {
      ...
      if (isMatch(token.token)) {
        if (m_ref->isFlag()) {
          ...
          auto
              valueRef = static_cast<detail::BoundValueRefBase *>( m_ref.get());
          ++remainingTokens;
          if (!remainingTokens)
...
clara::Parser makeCommandLineParser(ConfigData &config) {
  ...
  auto const setReporter = [&](std::string const &reporter) {
    IReporterRegistry::FactoryMap const
        &factories = getRegistryHub().getReporterRegistry().getFactories();
  ...
  auto cli
      = ExeName(config.processName)
          ...
          ["-w"]["--warn"]
              ("enable warnings")
          | Opt([&](bool flag) {
            config.showDurations = flag ? ShowDurations::Always
                                        : ShowDurations::Never;
          }, "yes|no")
          ["-d"]["--durations"]
              ("show test durations")
...
constexpr static std::size_t
    sigStackSize = 32768 >= MINSIGSTKSZ ? 32768 : MINSIGSTKSZ;
...
Floating::WithinUlpsMatcher WithinULP(double target, int maxUlpDiff) {
  return Floating::WithinUlpsMatcher(target,
                                     maxUlpDiff,
...
EqualsMatcher::EqualsMatcher(CasedString const &comparator) : StringMatcherBase(
    "equals",
    comparator) {}
...
StdString::ContainsMatcher Contains(std::string const &str,
                                    CaseSensitive::Choice caseSensitivity) {
  return StdString::ContainsMatcher(StdString::CasedString(str,
                                                           caseSensitivity));
}
StdString::EndsWithMatcher EndsWith(std::string const &str,
                                    CaseSensitive::Choice caseSensitivity) {
  return StdString::EndsWithMatcher(StdString::CasedString(str,
                                                           caseSensitivity));
}
StdString::StartsWithMatcher StartsWith(std::string const &str,
                                        CaseSensitive::Choice caseSensitivity) {
  return StdString::StartsWithMatcher(StdString::CasedString(str,
                                                             caseSensitivity));
}
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
  ...
  static GeneratorTracker &acquire(TrackerContext &ctx,
    ...
    if (TestCaseTracking::ITrackerPtr
        childTracker = currentTracker.findChild(nameAndLocation)) {
      assert(childTracker);
      assert(childTracker->isIndexTracker());
      tracker = std::static_pointer_cast<GeneratorTracker>(childTracker);
    } else {
      tracker = std::make_shared<GeneratorTracker>(nameAndLocation,
                                                   ctx,
                                                   &currentTracker);
...
void RunContext::assertionEnded(AssertionResult const &result) {
  ...
  static_cast<void>(m_reporter->assertionEnded(AssertionStats(result,
                                                              m_messages,
                                                              m_totals)));
...
IStreamingReporterPtr makeReporter(std::shared_ptr<Config> const &config) {
  ...
  auto const
      &listeners = Catch::getRegistryHub().getReporterRegistry().getListeners();
  ...
  int sync() override {
    if (pbase() != pptr()) {
      m_writer(std::string(pbase(),
                           static_cast<std::string::size_type>( pptr()
                               - pbase())));
      setp(pbase(), epptr());
    }
    return 0;
...
void TagAliasRegistry::add(std::string const &alias,
  ...
  CATCH_ENFORCE(m_registry.insert(std::make_pair(alias,
                                                 TagAlias(tag,
                                                          lineInfo))).second,
...
void setTags(TestCaseInfo &testCaseInfo, std::vector<std::string> tags) {
  ...
  for (auto const &tag : tags) {
    ...
    testCaseInfo.properties =
        static_cast<TestCaseInfo::SpecialProperties>( testCaseInfo.properties
...
TestCase::TestCase(ITestInvoker *testCase, TestCaseInfo &&info) : TestCaseInfo(
  ...
  switch (config.runOrder()) {
    case RunTests::InLexicographicalOrder:
      std::sort(sorted.begin(),
                sorted.end());
      break;
    case RunTests::InRandomOrder:seedRng(config);
...
ITracker &TrackerContext::startRun() {
  m_rootTracker = std::make_shared<SectionTracker>(NameAndLocation("{root}",
...
IndexTracker &IndexTracker::acquire(TrackerContext &ctx,
  ...
  if (ITrackerPtr childTracker = currentTracker.findChild(nameAndLocation)) {
    ...
    tracker = std::make_shared<IndexTracker>(nameAndLocation,
                                             ctx,
                                             &currentTracker,
                                             size);
    currentTracker.addChild(tracker);
...
void TestSpecParser::visitChar(char c) {
  if (m_mode == None) {
    switch (c) {
      ...
      case '\\': return escape();
      default: startNewMode(Name, m_pos);
        break;
    }
  }
  if (m_mode == Name) {
...
bool WildcardPattern::matches(std::string const &str) const {
  switch (m_wildcard) {
    case NoWildcard:return m_pattern == adjustCase(str);
    case WildcardAtStart:return endsWith(adjustCase(str), m_pattern);
    case WildcardAtEnd:return startsWith(adjustCase(str), m_pattern);
    case WildcardAtBothEnds:return contains(adjustCase(str), m_pattern);
 ...
 public:
  ...
  void print() {
    ...
    switch (result.getResultType()) {
      case ResultWas::Ok:printResultType(Colour::ResultSuccess, passedString());
      ...
      case ResultWas::DidntThrowException:
        ...
        printRemainingMessages();
        break;
      case ResultWas::Info:printResultType(Colour::None, "info");
        printMessage();
        printRemainingMessages();
        break;
      case ResultWas::Warning:printResultType(Colour::None, "warning");
        printMessage();
        printRemainingMessages();
        break;
...
void CompactReporter::sectionEnded(SectionStats const &_sectionStats) {
  if (m_config->showDurations() == ShowDurations::Always) {
    stream << getFormattedDuration(_sectionStats.durationInSeconds) << " s: "
  ...
  ConsoleAssertionPrinter(std::ostream &_stream,
    ...
    switch (result.getResultType()) {
      ...
      case ResultWas::Warning:messageLabel = "warning";
        break;
      case ResultWas::ExplicitFailure:passOrFail = "FAILED";
        colour = Colour::Error;
        if (_stats.infoMessages.size() == 1)
      ...
      case ResultWas::Unknown:
      case ResultWas::FailureBit:
      case ResultWas::Exception:passOrFail = "** internal error **";
        colour = Colour::Error;
        break;
    }
  }
...
class Duration {
  ...
  static const uint64_t
      s_nanosecondsInAMillisecond = 1000 * s_nanosecondsInAMicrosecond;
  static const uint64_t
      s_nanosecondsInASecond = 1000 * s_nanosecondsInAMillisecond;
...
void ConsoleReporter::benchmarkStarting(BenchmarkInfo const &info) {
  ...
  auto nameCol = Column(info.name).width(static_cast<std::size_t>(
                                             m_tablePrinter->columnInfos()[0].width
                                                 - 2 ));
...
bool JunitReporter::assertionEnded(AssertionStats const &assertionStats) {
  if (assertionStats.assertionResult.getResultType()
      == ResultWas::ThrewException && !m_okToFail)
...
void JunitReporter::writeSection(std::string const &className,
  ...
  if (!sectionNode.assertions.empty() ||
    ...
    xml.writeAttribute("time",
                       ::Catch::Detail::stringify(sectionNode.stats.durationInSeconds));

    writeAssertions(sectionNode);

    if (!sectionNode.stdOut.empty())
      xml.scopedElement("system-out").writeText(trim(sectionNode.stdOut),
                                                false);
    if (!sectionNode.stdErr.empty())
      xml.scopedElement("system-err").writeText(trim(sectionNode.stdErr),
                                                false);
  }
