...
public:
  ...
  explicit csvstream_exception(std::string msg) : msg(std::move(msg)){};
};
...
public:
  ...
  inline explicit csvstream(const std::string &filename, char delimiter = ',', bool strict = true);

  // Constructor from stream
  inline explicit csvstream(std::istream &is, char delimiter = ',', bool strict = true);
  ...
  inline std::vector<std::string> getheader() const;

  // Stream extraction operator reads one row
  inline csvstream &operator>>(std::map<std::string, std::string> &row);

  // Stream extraction operator reads one row, keeping column order
  inline csvstream &operator>>(std::vector<std::pair<std::string, std::string>> &row);

  inline csvstream &operator>>(std::vector<std::string> &row);
...
static bool read_csv_line(std::istream &is, std::vector<std::string> &data, char delimiter) {
...
csvstream::csvstream(const std::string &filename, char delimiter, bool strict)
    : filename(filename), is(fin), delimiter(delimiter), strict(strict), line_no(0) {
...
csvstream::csvstream(std::istream &is, char delimiter, bool strict)
    : filename("[no filename]"), is(is), delimiter(delimiter), strict(strict), line_no(0) {
  read_header();
}
...
csvstream &csvstream::operator>>(std::map<std::string, std::string> &row) {
  ...
  if (data.size() != header.size()) {
    auto msg = "Number of items in row does not match header. " + filename + ":L" + std::to_string(line_no) + " " +
...
csvstream &csvstream::operator>>(std::vector<std::pair<std::string, std::string>> &row) {
  ...
  if (row.size() != header.size()) {
    auto msg = "Number of items in row does not match header. " + filename + ":L" + std::to_string(line_no) + " " +
               "header.size() = " + std::to_string(header.size()) + " " + "row.size() = " + std::to_string(row.size()) +
               " ";
    throw csvstream_exception(msg);
  }
...
csvstream &csvstream::operator>>(std::vector<std::string> &row) {
  // Clear input row
  row.clear();
  row.resize(header.size());
