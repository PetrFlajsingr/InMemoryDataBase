...
public:
  ...
  template <typename T> CEREAL_RAPIDJSON_FORCEINLINE void Reserve(size_t count = 1) {
    // Expand the stack if needed
    if (CEREAL_RAPIDJSON_UNLIKELY(stackTop_ + sizeof(T) * count > stackEnd_))
      Expand<T>(count);
  }

  template <typename T> CEREAL_RAPIDJSON_FORCEINLINE T *Push(size_t count = 1) {
    Reserve<T>(count);
    return PushUnsafe<T>(count);
  }

  template <typename T> CEREAL_RAPIDJSON_FORCEINLINE T *PushUnsafe(size_t count = 1) {
  ...
  template <typename T> const T *End() const { return reinterpret_cast<T *>(stackTop_); }

  template <typename T> T *Bottom() { return reinterpret_cast<T *>(stack_); }

  template <typename T> const T *Bottom() const { return reinterpret_cast<T *>(stack_); }
