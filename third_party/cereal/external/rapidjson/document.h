...
template <typename CharType> struct GenericStringRef {
  ...
  //! Create string reference from \c const character array
                   ...
                   */
#endif
  template <SizeType N> GenericStringRef(const CharType (&str)[N]) CEREAL_RAPIDJSON_NOEXCEPT : s(str), length(N - 1) {}
   ...
   */
#endif
  GenericStringRef(const CharType *str, SizeType len) : s(str), length(len) { CEREAL_RAPIDJSON_ASSERT(s != 0); }
...
*/
template <typename CharType> inline GenericStringRef<CharType> StringRef(const std::basic_string<CharType> &str) {
...
template <typename T, typename Encoding = void, typename Allocator = void> struct IsGenericValueImpl : FalseType {};
...
public:
  ...
  */
  ...
  template <typename T>
  explicit GenericValue(T b,
                        CEREAL_RAPIDJSON_ENABLEIF((internal::IsSame<bool, T>))) CEREAL_RAPIDJSON_NOEXCEPT // See #472
#else
  ...
  */
  ...
  GenericValue &operator=(GenericValue &&rhs) CEREAL_RAPIDJSON_NOEXCEPT { return *this = rhs.Move(); }
#endif
  ...
  */
  template <typename SourceAllocator> bool operator==(const GenericValue<Encoding, SourceAllocator> &rhs) const {
   ...
   */
  template <typename T>
  friend CEREAL_RAPIDJSON_DISABLEIF_RETURN((internal::IsGenericValue<T>), (bool)) operator==(const T &lhs,
  ...
  */
  template <typename T>
  CEREAL_RAPIDJSON_DISABLEIF_RETURN((internal::NotExpr<internal::IsSame<typename internal::RemoveConst<T>::Type, Ch>>),
                                    (GenericValue &))
  operator[](T *name) {
  ...
  /*! \pre IsObject() == true
      ...
      \note Linear time complexity.
  */
  template <typename SourceAllocator> GenericValue &operator[](const GenericValue<Encoding, SourceAllocator> &name) {
  ...
  */
  ...
  /*!
      ...
      \note Linear time complexity.
  */
  template <typename SourceAllocator> bool HasMember(const GenericValue<Encoding, SourceAllocator> &name) const {
  ...
  */
  ...
  /*!
      ...
      \note Linear time complexity.
  */
  template <typename SourceAllocator> MemberIterator FindMember(const GenericValue<Encoding, SourceAllocator> &name) {
    CEREAL_RAPIDJSON_ASSERT(IsObject());
  ...
  */
  template <typename T>
  CEREAL_RAPIDJSON_DISABLEIF_RETURN((internal::OrExpr<internal::IsPointer<T>, internal::IsGenericValue<T>>),
                                    (GenericValue &))
  ...
  */
  ...
  /*!
      ...
      \return Iterator following the removed element. If the iterator pos refers to the last element, the End() iterator
     is returned. \note Linear time complexity.
  */
  ...
  */
  ...
  /*!
      \tparam T Either \c bool, \c int, \c unsigned, \c int64_t, \c uint64_t, \c double, \c float, \c const \c char*, \c
     std::basic_string<Ch>
  */
  template <typename T> bool Is() const { return internal::TypeHelper<ValueType, T>::Is(*this); }

  template <typename T> T Get() const { return internal::TypeHelper<ValueType, T>::Get(*this); }

  template <typename T> T Get() { return internal::TypeHelper<ValueType, T>::Get(*this); }

  template <typename T> ValueType &Set(const T &data) { return internal::TypeHelper<ValueType, T>::Set(*this, data); }
  ...
  template <typename Handler> bool Accept(Handler &handler) const {
    ...
    case kObjectType:
      ...
      for (ConstMemberIterator m = MemberBegin(); m != MemberEnd(); ++m) {
        ...
        if (CEREAL_RAPIDJSON_UNLIKELY(
                !handler.Key(m->name.GetString(), m->name.GetStringLength(), (m->name.data_.f.flags & kCopyFlag) != 0)))
          return false;
...
private:
  ...
  template <typename SourceAllocator> bool StringEqual(const GenericValue<Encoding, SourceAllocator> &rhs) const {
    CEREAL_RAPIDJSON_ASSERT(IsString());
...
public:
  ...
  */
  ...
  /*! \tparam parseFlags Combination of \ref ParseFlag (must not contain \ref kParseInsituFlag).
      \param str Read-only zero-terminated string to be parsed.
  */
  template <unsigned parseFlags> GenericDocument &Parse(const Ch *str) { return Parse<parseFlags, Encoding>(str); }
...
private:
  ...
  void Destroy() { CEREAL_RAPIDJSON_DELETE(ownAllocator_); }
...
/*!
   ...
   CEREAL_RAPIDJSON_HAS_CXX11_RANGE_FOR=1.
*/
