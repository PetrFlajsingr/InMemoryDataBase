...
enum WriteFlag {
  ...
  kWriteDefaultFlags = CEREAL_RAPIDJSON_WRITE_DEFAULT_FLAGS //!< Default write flags. Can be customized by defining
...
#undef Z16
    ...
    while (ScanWriteUnescapedString(is, length)) {
      ...
      if (!TargetEncoding::supportUnicode && static_cast<unsigned>(c) >= 0x80) {
        ...
        if (codepoint <= 0xD7FF || (codepoint >= 0xE000 && codepoint <= 0xFFFF)) {
          ...
          PutUnsafe(*os_, hexDigits[(trail)&15]);
                 ...
                 CEREAL_RAPIDJSON_UNLIKELY(escape[static_cast<unsigned char>(c)])) {
        is.Take();
        PutUnsafe(*os_, '\\');
      ...
      } else if (CEREAL_RAPIDJSON_UNLIKELY(
