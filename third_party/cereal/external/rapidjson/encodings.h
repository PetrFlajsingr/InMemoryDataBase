...
template <typename CharType = char> struct UTF8 {
  ...
  template <typename OutputStream> static void Encode(OutputStream &os, unsigned codepoint) {
    if (codepoint <= 0x7F)
...
#undef TAIL
  ...
  template <typename InputStream, typename OutputStream> static bool Validate(InputStream &is, OutputStream &os) {
...
#undef TAIL
  ...
  template <typename InputByteStream> static CharType TakeBOM(InputByteStream &is) {
  ...
  template <typename OutputByteStream> static void PutBOM(OutputByteStream &os) {
...
template <typename CharType = wchar_t> struct UTF16LE : UTF16<CharType> {
  ...
  template <typename InputByteStream> static CharType Take(InputByteStream &is) {
    CEREAL_RAPIDJSON_STATIC_ASSERT(sizeof(typename InputByteStream::Ch) == 1);
  ...
  template <typename OutputByteStream> static void Put(OutputByteStream &os, CharType c) {
    CEREAL_RAPIDJSON_STATIC_ASSERT(sizeof(typename OutputByteStream::Ch) == 1);
