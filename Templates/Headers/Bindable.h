 ...
 */
template <typename T> class Binder : public Observer {
public:
  /**
   *
   ...
   */
  Binder(Observable<T> *bind, Observable<T> *to) : bind(bind), to(to) { to->addObserver(this); }
  /**
   * Remove observer from observed value.
   */
  virtual ~Binder() { to->deleteObserver(this); }
  /**
...
template <typename T, typename U,
          typename Ret = typename std::conditional<std::is_floating_point<T>::value, T, U>::type>
class Binding : public Bindable<Ret>, public Observer {
...
public:
   ...
   */
  void update(ObservableBase const *) override { *this = compute(a, b); }
 ...
 */
template <typename T, typename U> class TransformBinding : public Bindable<T>, public Observer {
...
public:
   ...
   */
  TransformBinding(const std::shared_ptr<Bindable<U>> &a, const TransFunc &transform) : transform(transform), a(a) {
    *this = transform(*a);
    a->addObserver(this);
  }
  /**
   ...
   */
  void update(ObservableBase const *) override { *this = transform(*a); }
 ...
 */
template <typename T> class Bindable : public Observable<T>, public std::enable_shared_from_this<Bindable<T>> {
public:
  Bindable() = default;
   ...
   */
  template <typename U> void bindBidirectional(Bindable<U> *to, Converter<U, T> *converter) {
    // TODO
   ...
   * @param to Bindable to bind to
   */
  void bind(Bindable<T> *to) { binders.emplace_back(this, to); }
  /**
  ...
  void unbind(Bindable<T> *from) {
    if (auto it = std::find(binders.begin(), binders.end(), from); it != binders.end()) {
      binders.remove(it);
    }
  }
  using Observable<T>::operator=;
   ...
   */
  template <typename U, typename Ret = typename std::conditional<std::is_floating_point<T>::value, T, U>::type>
  std::shared_ptr<Binding<T, U>> add(std::shared_ptr<Bindable<U>> obs) {
    ...
    return std::make_shared<Binding<T, U>>(
        this->shared_from_this(), obs,
        [](std::shared_ptr<Observable<T>> a, std::shared_ptr<Observable<U>> b) -> Ret { return (T)*a + (U)*b; });
  }
  /**
  ...
  std::shared_ptr<Binding<T, U>> diff(std::shared_ptr<Bindable<U>> obs) {
    ...
    return std::make_shared<Binding<T, U>>(
        this->shared_from_this(), obs,
        [](std::shared_ptr<Bindable<T>> a, std::shared_ptr<Bindable<U>> b) -> Ret { return (T)*a - (U)*b; });
  }
  /**
   ...
   */
  template <typename U> std::shared_ptr<TransformBinding<U, T>> transform(std::function<U(T)> func) {
    static_assert(std::is_fundamental<U>::value && std::is_fundamental<T>::value);
    return std::make_shared<TransformBinding<U, T>>(this->shared_from_this(), func);
  }
