...
public:
   ...
   */
  template <typename T> void registerMsg(MessageReceiver *receiver) {
   ...
   * @param receiver listener
   */
  template <typename T> void unregisterMsg(MessageReceiver *receiver) {
  ...
  void unregister(MessageReceiver *receiver) {
    std::for_each(commands.begin(), commands.end(),
                  [receiver](std::pair<const std::type_index, std::vector<MessageReceiver *>> &record) {
                    if (auto it = std::find(record.second.begin(), record.second.end(), receiver);
                        it != record.second.end()) {
   ...
   */
  template <typename T> void dispatch(std::shared_ptr<T> command) {
    static_assert(std::is_base_of<Message, T>{});
    auto receivers = commands[typeid(*command.get())];
    std::for_each(receivers.begin(), receivers.end(),
                  [&command](MessageReceiver *receiver) { receiver->receive(command); });
  }
