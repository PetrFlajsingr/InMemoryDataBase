 ...
 */
template <typename T> class DataMessage : public Message {
protected:
  T data;
 ...
 */
template <typename T> class FncMessage : public Message {
protected:
  T fnc;
...
public:
  ...
  [[nodiscard]] T getFnc() const { return fnc; }
};
/**
...
public:
  ...
  std::function<void()> &getOnDone() { return onDone; }
...
public:
  explicit StdinMsg(const std::string &data) { DataMessage::data = data; }
};
/**
 * End UI notification.
 */
...
public:
  explicit Download(std::pair<std::string, std::string> data) { DataMessage::data = std::move(data); }
  Download(const std::string &url, const std::string &dest) { DataMessage::data = std::make_pair(url, dest); }
};
/**
...
public:
  explicit Print(const std::string &data) { DataMessage::data = data; }
};
/**
...
enum class DownloadState { started, finished, failed };
/**
 * Provide download progress information.
 */
class DownloadProgress : public DataMessage<std::pair<DownloadState, std::string>> {
public:
  explicit DownloadProgress(const std::pair<DownloadState, std::string> &data) { DataMessage::data = data; }
  DownloadProgress(DownloadState state, const std::string &file) { DataMessage::data = std::make_pair(state, file); }
};
