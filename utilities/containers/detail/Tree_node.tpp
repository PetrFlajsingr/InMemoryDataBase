...
Leaf<T, ChildCount>::Leaf(const Leaf &other) : value(other.value), parent(other.parent) {}
...
template <typename T, unsigned int ChildCount>
Leaf<T, ChildCount>::Leaf(Leaf &&other) noexcept : value(std::move(other.value)), parent(other.parent) {}
template <typename T, unsigned int ChildCount>
...
void Node<T, ChildCount>::setChildrenValues(const_reference_type value, NodeType nodeType) {
  if (nodeType == NodeType::Leaf) {
    std::generate(children.begin(), children.end(),
                  [&value] { return std::move(std::make_unique<Leaf<T, ChildCount>>(value)); });
  } else if (nodeType == NodeType::Node) {
