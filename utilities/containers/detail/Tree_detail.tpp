...
TreeIterator<T, ChildCount>::TreeIterator() : root(nullptr), current(nullptr) {}
template <typename T, unsigned int ChildCount>
TreeIterator<T, ChildCount>::TreeIterator(TreeIterator::LeafType *root) : root(root), current(root) {
  buildQueue(root);
  operator++();
}
template <typename T, unsigned int ChildCount>
TreeIterator<T, ChildCount>::TreeIterator(const TreeIterator::iterator &other)
    : root(other.root), current(other.root), queue(other.queue) {}
template <typename T, unsigned int ChildCount>
typename TreeIterator<T, ChildCount>::iterator &
TreeIterator<T, ChildCount>::operator=(const TreeIterator::iterator &other) {
  if (&other == this) {
    return *this;
  }
  root = other.root;
...
template <typename T, unsigned int ChildCount>
void TreeIterator<T, ChildCount>::buildQueue(TreeIterator::LeafType *node) {
  if (node == nullptr) {
    return;
  }
  queue.push(node);
