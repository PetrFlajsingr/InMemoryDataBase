...
bool geo::BoundingBox<Dimensions>::operator==(const geo::BoundingBox<Dimensions> &rhs) const {
...
geo::RelativePosition geo::BoundingBox<Dimensions>::contains(const geo::BoundingBox<Dimensions> &aabb) const {
  if constexpr (Dimensions == 2) {
    ...
    const bool minIn = p1.x < aabb.p1.x && p1.y < aabb.p1.y && p1.z < aabb.p1.z && p2.x > aabb.p1.x &&
                       p2.y > aabb.p1.y && p2.z > aabb.p1.z;
    const bool maxIn = p2.x > aabb.p2.x && p2.y > aabb.p2.y && p2.z > aabb.p2.z && p1.x < aabb.p2.x &&
                       p1.y < aabb.p2.y && p1.z < aabb.p2.z;
    if (minIn && maxIn) {
      return RelativePosition::Inside;
    }
    if (minIn || maxIn) {
